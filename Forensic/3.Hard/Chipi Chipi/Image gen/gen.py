...
if painting_len < 750:
	# plato_painting = imageio.v2.imread('Plato.png')           # 390
	plato_painting = imageio.v2.imread('p1.png')              # 750
...
#-------------- if image has 4 channels
	# Assuming piet_painting is an RGB image (3 channels)
	alpha_channel = np.ones((piet_painting.shape[0], piet_painting.shape[1], 1), dtype=np.uint8) * 255
	piet_painting_with_alpha = np.concatenate([piet_painting, alpha_channel], axis=-1)
	# Now both images have 4 channels

	#--------------Breaking the image into 2 parts
	# if painting_len % 2 == 0:
	# 	mid = painting_len // 2
	# else:
	# 	mid = painting_len // 2 + 1

	# plato_painting[0:12, 0:mid, :] = piet_painting_with_alpha[:, 0:mid, :]
	# plato_painting[738:750, 0:mid, :] = piet_painting_with_alpha[:, mid-1:painting_len, :]
	#---------------------------------------
	
	plato_painting[0:12, 0:painting_len, :] = piet_painting_with_alpha[:, 0:painting_len, :]
	# plato_painting[375:387, 0:painting_len, :] = piet_painting_with_alpha[:, 0:painting_len, :]
#---------------------------------------

	plato_img = Image.fromarray(plato_painting)
	imageio.imwrite('piet_coded.png', plato_img)
